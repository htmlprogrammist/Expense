# Expense

Application that allows you to track your finances, daily budget etc.

# Description

In this project I demonstrated the knowledge of:
- UIKit
    - CompositionalLayout
    - DiffableDataSource
- Clean Swift
- Core Data & CloudKit
- Notification Center, Timer
- Core Animation
- Push-notifications
- Working with `.csv` format
- Testing (Unit/Integration/UI)

# Requirements

- The interface is drawn in UIKit
- The app looks correct for iPhone SE (2016) and above
- Minimum version: iOS 13
- Using the MVC architectural pattern is prohibited
- To cover the code with tests, you must use XCTest. Tests should cover at least half of the written code
- For permanent data storage, you need to use only Core Data
- Synchronization with iCloud must be implemented
- No storyboard or Xib is allowed
- The project should use the [SwiftLint](https://github.com/realm/SwiftLint) linter

---
# Expense

Приложение для учёта финансов, достижения целей и составления бюджетов на день и месяц

# Описание

В данном проекте я продемонстрировал знания:

- UIKit
    - CompositionalLayout
    - DiffableDataSource
- Clean Swift
- Core Data & CloudKit
- Notification Center, Timer
- Core Animation
- Push-уведомления
- Работа с форматом `.csv`
- Testing (Unit/Integration/UI)

# Требования

- Для отрисовки интерфейса используется UIKit
- Приложение корректно отображается на iPhone SE (2016) и выше
- Минимальная версия: iOS 13
- Использование архитектурного паттерна MVC запрещено
- Для постоянного хранения данных необходимо использовать Core Data
- Должна быть реализована синхронизация с iCloud
- Использование Storyboard/Xib запрещено
- Для покрытия кода тестами необходимо использовать XCTest. Тесты должны покрывать не менее половины написанного кода
- В проекте должен быть использован линтер [SwiftLint](https://github.com/realm/SwiftLint)
